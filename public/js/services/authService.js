...
class AuthServiceClass {
  ...
  async login(username, password) {
    try {
      ...
      if (user.requires2FA && user.tempToken) {
        window.location.href = `/login-2fa?tempToken=${encodeURIComponent(user.tempToken)}`;
        return user;
      }

      AppState.set('user', user);
      EventBus.emit('toast:show', { message: `Selamat datang kembali, ${user.username}!`, type: 'success' });
      Router.navigate(user.role === 'admin' || user.role === 'superadmin' ? '/dashboard' : '/dashboard/upload');
      return user;
    } catch (err) {
