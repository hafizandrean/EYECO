...
export class PromotionService {
  ...
  public static async evaluateDetection(detection: IAiDetection): Promise<void> {
    try {
      ...
      const existingReport = await ReportModel.findOne({ sourceDetectionId: detection._id, deletedAt: null }).exec() as any;
      if (existingReport) {
        ...
        const claimedReport = await ReportModel.findOneAndUpdate(
          {
      ...
      const nextReportId = await getNextSequence('reportId', ReportModel);

      let initialAiStatus: 'Indikasi Tinggi' | 'Indikasi Sedang' | 'Indikasi Rendah' | 'Tidak Terindikasi' = 'Tidak Terindikasi';
      if (detection.severity === 'CRITICAL' || detection.severity === 'HIGH') initialAiStatus = 'Indikasi Tinggi';
      else if (detection.severity === 'MEDIUM') initialAiStatus = 'Indikasi Sedang';
      else if (detection.severity === 'LOW') initialAiStatus = 'Indikasi Rendah';
      ...
      let newReportDoc: any = null;
      try {
        newReportDoc = await ReportModel.create({
          id: nextReportId,
        ...
        if (createErr.code === 11000 || (createErr.message && createErr.message.includes('E11000'))) {
          ...
          const existing = await ReportModel.findOne({ sourceDetectionId: detection._id, deletedAt: null }).exec();
          if (existing) {
            detection.status = 'PROMOTED';
  ...
  public static async runPeriodicRecoveryJob(): Promise<void> {
   ...
   */
  public static async recoverExpiredAnalysis(): Promise<number> {
    const now = new Date();
    try {
      // 1. Move expired attempts with count < 3 to REANALYSIS_PENDING
      const resPending = await ReportModel.updateMany(
        {
          analysisState: 'PROCESSING',
      ...
      const resFailed = await ReportModel.updateMany(
        {
          analysisState: 'PROCESSING',
          analysisLeaseExpiresAt: { $lte: now },
   ...
   */
  public static async processPendingReanalysis(): Promise<number> {
    const now = new Date();
    try {
      ...
      for (const report of pendingReports) {
        if (!report.sourceDetectionId) continue;
        const detection = await AiDetectionModel.findById(report.sourceDetectionId).exec();
  ...
  private static async runAiAnalysisAndFinalize(reportDoc: any, claimToken: string, detection: IAiDetection, imagePath: string): Promise<void> {
    let snapshot = null;
    let decisionResult = null;
    try {
      const fullPath = path.isAbsolute(imagePath) ? imagePath : path.join(process.cwd(), 'public', imagePath.startsWith('/') ? imagePath.slice(1) : imagePath);
      if (fs.existsSync(fullPath)) {
        const aiRes = await aiEngine.analyzeImage(fullPath, { reportId: reportDoc.id });
    ...
    const snapObj = snapshot as any;
    const decObj = decisionResult as any;
    ...
    if (!updatedReport) {
      ...
      if (snapObj && snapObj._id) {
        try {
          const isAlreadyActive = await ReportModel.exists({ activeSnapshotId: snapObj._id });
          if (!isAlreadyActive) {
    ...
    if (createdReport.analysisState === 'READY' && createdReport.activeSnapshotId) {
      await NotificationDispatcher.dispatch(createdReport);
    }
  }
