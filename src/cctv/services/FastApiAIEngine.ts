...
export class FastApiAIEngine implements IAIEngine {
  ...
  private predictUrl = 'http://localhost:8000/api/v1/predict';
  private reloadUrl = 'http://localhost:8000/api/v1/reload_model';
  
  // Model state variables
  private activeModelId = 'yolov8-river-v1.0';
  private deploymentId = 'deploy-init-000';
  private deploymentGeneration = 1;
  ...
  private static inferenceMetricsBuffer: any[] = [];
  ...
  public async initialize(modelPath: string): Promise<void> {
    ...
    const modelId = path.basename(modelPath, '.pt');
    ...
    try {
      ...
      let checksum = '';
      let version = '1.0';
      let recordObj: any = null;
      
      try {
        const modelRecord = await AiModelModel.findOne({ id: modelId }).exec();
        if (modelRecord) {
          version = modelRecord.version;
          recordObj = modelRecord.toObject() as any;
      ...
      if (fs.existsSync(localWeightsPath) && !checksum) {
        try {
          const fileBuffer = fs.readFileSync(localWeightsPath);
  ...
  public async detect(frame: ICapturedFrame): Promise<IDetectionResult[]> {
    ...
    try {
      ...
      try {
        const settings = await SystemSettingsModel.findOne({ key: 'ai.rules' }).exec();
      ...
      formData.append('traceId', crypto.randomUUID());
      formData.append('correlationId', crypto.randomUUID());
      ...
      if (data && data.objects) {
        return data.objects.map((obj: any) => ({
          ...
          bbox: obj.geometry.value as [number, number, number, number],
  ...
  private compareVersions(current: string, required: string): boolean {
    if (!required || required.toLowerCase() === 'any' || required.trim() === '') return true;
    ...
    for (let i = 0; i < Math.max(currParts.length, reqParts.length); i++) {
      ...
      if (currPart > reqPart) return true;
      if (currPart < reqPart) return false;
    }
    return true; // equal
  }
   ...
   */
  private resolveAbsolutePath(imagePath: string): string {
