...
export class InferenceQueue {
  ...
  private static cachedMaxQueueSize = 50;
  private static lastSettingsSync = 0;
  
  public static minWorkers = 2;
  public static maxWorkers = 8;
  private static activeWorkers = 0;
  private static workerIdSeq = 0;
  private static accepting = true;

  // Observability metrics
  public static droppedFramesCount = 0;
  public static expiredFramesCount = 0;
  public static totalProcessedCount = 0;
  private static busyWorkers = 0;
  private static waitingTimes: number[] = [];
  private static processedTimestamps: number[] = [];

  // Default priority to numeric weight mapping
  private static readonly priorityWeightsRecord: Record<PriorityType, number> = {
    CRITICAL: 100,
    HIGH: 80,
    NORMAL: 50,
    LOW: 20
  ...
  private static spawnWorker() {
    ...
    (async () => {
      ...
      while (this.accepting || this.queue.length > 0) {
        ...
        if (this.waitingTimes.length > 100) this.waitingTimes.shift();

        try {
          this.busyWorkers++;
          const result = await InferenceService.executeInference(item.frame);
   ...
   */
  private static async getMaxSize(): Promise<number> {
    const now = Date.now();
    if (now - this.lastSettingsSync > 10000) {
      this.lastSettingsSync = now;
      try {
        const setting = await SystemSettingsModel.findOne({ key: 'ai.queue.maxSize' }).exec();
  ...
  public static getActiveWorkers(): number {
    return this.activeWorkers;
  }

  public static getBusyWorkers(): number {
    return this.busyWorkers;
  }

  public static getFpsThroughput(): number {
    const now = Date.now();
  ...
  public static getAverageWaitingTimeMs(): number {
    if (this.waitingTimes.length === 0) return 0;
    const sum = this.waitingTimes.reduce((a, b) => a + b, 0);
    return Math.round(sum / this.waitingTimes.length);
  }

  public static getWorkerUtilization(): number {
    if (this.activeWorkers === 0) return 0;
