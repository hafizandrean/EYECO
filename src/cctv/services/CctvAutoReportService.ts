...
export class CctvAutoReportService {
  ...
  private static async processCameraSnapshot(camera: ICctv): Promise<void> {
    try {
      const lastCapturePath = path.resolve(__dirname, '../../../public/uploads/last_capture.jpg');
      ...
      try {
        ...
        aiStatus = aiAnalysis.decision.status as any;
  ...
  public static async processDetection(
    ...
    try {
      ...
      const personDetections = detection.detections.filter(
        d => d.class === 'person' && d.confidence >= 0.5
      );
      ...
      let aiStatus: 'TINGGI' | 'SEDANG' | 'RENDAH' | 'Tidak Terindikasi' = 'TINGGI';
      if (maxConfidence >= 0.8) aiStatus = 'TINGGI';
      else if (maxConfidence >= 0.6) aiStatus = 'SEDANG';
      else aiStatus = 'RENDAH';
