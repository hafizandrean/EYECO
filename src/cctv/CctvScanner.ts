...
export class CctvScanner {
    ...
    try {
      ...
      if (forcedMode && forcedMode !== 'AUTO') {
        ...
        if (forcedMode === 'RTSP') {
          ...
          let path = '/live';
          if (vendorHint === 'HIKVISION') path = '/Streaming/Channels/101';
          else if (vendorHint === 'DAHUA') path = '/cam/realmonitor?channel=1&subtype=0';
          else if (vendorHint === 'KRISBOW') path = '/live/ch0';
  ...
  private static probePort(host: string, port: number, timeout: number): Promise<boolean> {
