...
router.get('/join-requests', authMiddleware, roleGuard(['admin']), async (req, res) => {
  try {
    const user = await getLoggedInUser(req as any);
    if (!user) return res.status(401).json({ error: 'Belum masuk' });
    ...
    const users = await UserModel.find({ id: { $in: userIds } }).select('id username name email').lean().exec();
...
router.post('/join-requests/:id/approve', authMiddleware, roleGuard(['admin']), async (req, res) => {
  try {
    const user = await getLoggedInUser(req as any);
    if (!user || !user.workspaceId) return res.status(403).json({ error: 'Akses ditolak' });

    const reqId = req.params.id;
    const joinReq = await JoinRequestModel.findOne({ _id: reqId, workspaceId: user.workspaceId, status: 'PENDING' });
    
    if (!joinReq) return res.status(404).json({ error: 'Permintaan tidak ditemukan' });
    ...
    await UserModel.updateOne(
      { id: joinReq.userId },
      { $addToSet: { workspaceIds: user.workspaceId } }
    );
...
router.post('/join-requests/:id/reject', authMiddleware, roleGuard(['admin']), async (req, res) => {
  try {
    const user = await getLoggedInUser(req as any);
    if (!user || !user.workspaceId) return res.status(403).json({ error: 'Akses ditolak' });
...
router.patch('/users/:id/approve', authMiddleware, roleGuard(['admin']), async (req, res) => {
  try {
    const admin = await getLoggedInUser(req as any);
    if (!admin?.workspaceId) return res.status(403).json({ error: 'Admin belum diassign ke workspace' });
    const targetId = parseInt(req.params.id);
    if (isNaN(targetId)) return res.status(400).json({ error: 'ID tidak valid' });

    const target = await UserModel.findOne({ id: targetId, role: 'user', workspaceIds: admin.workspaceId }).lean().exec();
    if (!target) return res.status(404).json({ error: 'User tidak ditemukan di workspace ini' });

    const updated = await UserRepository.updateStatus(targetId, 'APPROVED', admin.workspaceId);
...
router.patch('/users/:id/reject', authMiddleware, roleGuard(['admin']), async (req, res) => {
  try {
    const admin = await getLoggedInUser(req as any);
    if (!admin?.workspaceId) return res.status(403).json({ error: 'Admin belum diassign ke workspace' });
    const targetId = parseInt(req.params.id);
    ...
    const updated = await UserRepository.updateStatus(targetId, 'REJECTED', admin.workspaceId);
    if (!updated) return res.status(404).json({ error: 'User tidak ditemukan' });
