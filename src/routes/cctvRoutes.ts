...
router.get('/', async (req, res) => {
  try {
    ...
    const workspaceId = user.role === 'superadmin' ? undefined : (user.workspaceId || -1);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const decryptedPassword = (user.role === 'admin' || user.role === 'superadmin') && c.password
...
router.post('/scan', async (req, res) => {
  try {
    const user = await getLoggedInUser(req);
    if (!user) return res.status(401).json({ error: 'Belum masuk' });
    if (user.role !== 'admin' && user.role !== 'superadmin') return res.status(403).json({ error: 'Akses ditolak' });
...
router.post('/', async (req, res) => {
  try {
    ...
    const workspaceId = user.role === 'superadmin' ? req.body.workspaceId : user.workspaceId;
    if (!workspaceId) return res.status(400).json({ error: 'Workspace ID wajib ditentukan.' });

    const newCctv = await CctvRepository.add({ ...req.body, workspaceId }, user.id);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const workspaceId = user.role === 'superadmin' ? undefined : user.workspaceId;
