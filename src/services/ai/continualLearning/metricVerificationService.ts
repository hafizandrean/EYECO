...
import { MetricVerificationResultModel, IMetricVerificationResult } from '../../../database/models/MetricVerificationResult';
import { IMlExecutionContext, toRelativePosixPath } from './MlExecutionContext';

export interface ICreateMetricVerificationParams {
  evaluatorExecutionResultId: string;
  evaluationPolicyId?: string;
  context?: IMlExecutionContext;
}

export function computeBoxIoU(boxA: number[], boxB: number[]): number {
...
export function compute101PointAp(recalls: number[], precisions: number[]): number {
  if (!recalls || !precisions || recalls.length === 0) return 0.0;
...
export function computeIndependentMetrics(predItems: any[], gtItems: any[]): any {
  ...
  if (allClasses.size === 0) allClasses.add('plastic_bag');

  const iouThresholds = Array.from({ length: 10 }, (_, i) => Number((0.50 + i * 0.05).toFixed(2)));
  const classAps: Record<string, number[]> = {};
  let overallTp = 0;
  let overallFp = 0;
  let overallGt = 0;
  let smallGtCount = 0;
  ...
  for (const cls of Array.from(allClasses)) {
    ...
    for (const iouThresh of iouThresholds) {
      const gtBoxesDict: Record<string, Array<{ bbox: number[]; matched: boolean; isSmall: boolean }>> = {};
      for (const gt of gtItems) {
        const key = gt.goldenItemId || gt.imageHash || gt.imagePath;
        if (key) {
          const clsAnns = (gt.annotations || []).filter((a: any) => (a.className || 'plastic_bag') === cls);
          gtBoxesDict[key] = clsAnns.map((a: any) => {
      ...
      const detections: Array<{ key: string; bbox: number[]; confidence: number }> = [];
      for (const pred of predItems) {
      ...
      if (iouThresh === 0.50) overallGt += totalClsGt;
      ...
      for (const det of detections) {
        ...
        if (bestIou >= iouThresh && bestIdx >= 0) {
          ...
          if (iouThresh === 0.50) {
            overallTp += 1;
            if (gtBoxes[bestIdx].isSmall) smallTpCount += 1;
          }
        } else {
          tpList.push(0);
          fpList.push(1);
          if (iouThresh === 0.50) overallFp += 1;
        }
      }

      if (iouThresh === 0.50) {
        Object.values(gtBoxesDict).forEach(boxes => {
          boxes.forEach(b => { if (b.isSmall) smallGtCount += 1; });
        });
      }
...
export class MetricVerificationService {
  public async createMetricVerificationResult(
    params: ICreateMetricVerificationParams
  ): Promise<IMetricVerificationResult> {
    ...
    let evalRecord = await EvaluatorExecutionResultModel.findOne({ executionId: evaluatorExecutionResultId }).exec();
    if (!evalRecord && mongoose.Types.ObjectId.isValid(evaluatorExecutionResultId)) {
      evalRecord = await EvaluatorExecutionResultModel.findById(evaluatorExecutionResultId).exec();
    }
    if (!evalRecord) {
      const err: any = new Error(`EVALUATOR_EXEC_RESULT_NOT_FOUND: EvaluatorExecutionResult '${evaluatorExecutionResultId}' not found.`);
      err.status = 404;
      throw err;
    }
    ...
    if (artifactRoot) {
      ...
      const metricsPathResolved = path.resolve(evalRecord.evaluationMetricsFilePath);
      if (!metricsPathResolved.startsWith(resolvedRoot + path.sep) && metricsPathResolved !== resolvedRoot) {
        const err: any = new Error(`TEST_ARTIFACT_PATH_ESCAPE: Path '${evalRecord.evaluationMetricsFilePath}' escapes test artifact root '${artifactRoot}'.`);
        err.status = 403;
        throw err;
      }
    }
    ...
    if (!fs.existsSync(evalRecord.evaluationMetricsFilePath)) {
      const err: any = new Error(`EVALUATION_METRICS_FILE_NOT_FOUND: Primary metrics file '${evalRecord.evaluationMetricsFilePath}' not found.`);
      err.status = 422;
      throw err;
    }

    const primaryMetricsFile = JSON.parse(fs.readFileSync(evalRecord.evaluationMetricsFilePath, 'utf-8'));
    ...
    const candPredData = fs.existsSync(evalRecord.candidatePredictionManifestPath)
      ? JSON.parse(fs.readFileSync(evalRecord.candidatePredictionManifestPath, 'utf-8'))
      : { items: [] };

    const gtData = fs.existsSync(evalRecord.groundTruthManifestPath)
      ? JSON.parse(fs.readFileSync(evalRecord.groundTruthManifestPath, 'utf-8'))
      : { items: [] };
    ...
    const policyHash = crypto.createHash('sha256').update(evaluationPolicyId).digest('hex');
    const scriptHash = evalRecord.evaluatorScriptHash || crypto.createHash('sha256').update('independent-map-engine-v1').digest('hex');
    const runtimeHash = crypto.createHash('sha256').update(`node-${process.version}`).digest('hex');
    const primaryMetricsHash = evalRecord.evaluationMetricsFileHash || crypto.createHash('sha256').update(fs.readFileSync(evalRecord.evaluationMetricsFilePath)).digest('hex');
    ...
    const resultHash = crypto.createHash('sha256').update(canonicalString).digest('hex');

    // 6. Atomically create immutable MetricVerificationResult record
    const verificationId = `verif-metric-${Date.now()}-${crypto.randomBytes(3).toString('hex')}`;
    const metricVerifRecord = await MetricVerificationResultModel.create({
      verificationId,
...
export const metricVerificationService = new MetricVerificationService();
