import { AiDatasetCandidateModel, IAiDatasetCandidate, TargetModelType } from '../../../database/models/AiDatasetCandidate';
import { AiSnapshotModel } from '../../../database/models/AiSnapshot';
import { AiValidationLogModel } from '../../../database/models/AiValidationLog';
import { AiDatasetVersionModel, IAiDatasetVersion, IDatasetManifestItem, DatasetSplitType, DatasetVersionStatus, TrainingEligibilityStatus } from '../../../database/models/AiDatasetVersion';
...
export class DatasetBuilder {
  public static readonly BUILDER_VERSION = 'v3.0.0';
  public static readonly SPLIT_STRATEGY_VERSION = 'v1.0-deterministic-group';
  public static readonly SPLIT_SEED = 'eyeco-seed-2026';
  ...
  private sanitizeManifestItemsForHash(items: IDatasetManifestItem[]): any[] {
  ...
  public buildTransitiveLineageGroups(candidates: IAiDatasetCandidate[]): Map<string, IAiDatasetCandidate[]> {
    const uf = new UnionFind();
    const candidateIdMap = new Map<string, IAiDatasetCandidate>();
    ...
    for (const cand of candidates) {
      ...
      for (let k = 0; k < keys.length; k++) {
        uf.union(cId, keys[k]);
        if (k > 0) uf.union(keys[k - 1], keys[k]);
      }
    }

    const groupMap = new Map<string, IAiDatasetCandidate[]>();
    for (const cand of candidates) {
      const cId = cand._id.toString();
  ...
  public async buildDatasetVersion(params: {
    ...
    try {
      await mongoSession.withTransaction(async () => {
        ...
        if (candidates.length === 0) {
          const err: any = new Error(`No APPROVED candidates available for building dataset version (targetModel: ${params.targetModel}).`);
        ...
        await AiDatasetCandidateModel.updateMany(
          { _id: { $in: candIds }, approvalStatus: 'APPROVED' },
          { $set: { approvalStatus: 'RESERVED_FOR_BUILD' } },
          { session: mongoSession }
        ).exec();
        ...
        const groupKeys = Array.from(groupMap.keys()).sort((a, b) => {
          const hashA = crypto.createHash('sha256').update(a + DatasetBuilder.SPLIT_SEED).digest('hex');
          const hashB = crypto.createHash('sha256').update(b + DatasetBuilder.SPLIT_SEED).digest('hex');
          return hashA.localeCompare(hashB);
        });

        // 3. Deterministic Grouped Allocation Engine
        const groupSplits = new Map<string, DatasetSplitType>();
        const totalGroups = groupKeys.length;

        for (let i = 0; i < totalGroups; i++) {
          const gKey = groupKeys[i];
          let split: DatasetSplitType = 'TRAIN';

          if (totalGroups >= 3) {
            // Allocate at least 1 group to TRAIN, VAL, TEST if totalGroups >= 3
            if (i === 0) split = 'TRAIN';
            else if (i === 1) split = 'VAL';
            else if (i === 2) split = 'TEST';
            else {
              const hashVal = parseInt(crypto.createHash('sha256').update(gKey + DatasetBuilder.SPLIT_SEED).digest('hex').slice(0, 8), 16) % 100;
              if (hashVal < 70) split = 'TRAIN';
              else if (hashVal < 85) split = 'VAL';
              else split = 'TEST';
            }
          } else {
        ...
        const manifestItems: IDatasetManifestItem[] = [];
        const splitCounts = { train: 0, val: 0, test: 0, total: 0 };

        const splitByGroupKey: Record<string, DatasetSplitType> = {};
        const splitByImageHash: Record<string, DatasetSplitType> = {};
        const splitByIncidentId: Record<string, DatasetSplitType> = {};
        ...
        for (const [groupRootKey, candGroup] of groupMap.entries()) {
          ...
          for (const cand of candGroup) {
            const snapshot = await AiSnapshotModel.findById(cand.snapshotId, null, { session: mongoSession }).exec();
            const validationLog = cand.validationLogId ? await AiValidationLogModel.findById(cand.validationLogId, null, { session: mongoSession }).exec() : null;

            // Audit Cross-Split Leaks
            if (splitByGroupKey[groupRootKey] && splitByGroupKey[groupRootKey] !== split) crossSplitGroupLeaks++;
            else splitByGroupKey[groupRootKey] = split;

            if (splitByImageHash[cand.inputImageHash] && splitByImageHash[cand.inputImageHash] !== split) crossSplitHashLeaks++;
            else splitByImageHash[cand.inputImageHash] = split;

            if (cand.incidentId) {
              if (splitByIncidentId[cand.incidentId] && splitByIncidentId[cand.incidentId] !== split) crossSplitIncidentLeaks++;
              else splitByIncidentId[cand.incidentId] = split;
            }
            ...
            if (validationLog && Array.isArray(validationLog.correctedObjects) && validationLog.correctedObjects.length > 0) {
              for (const obj of (validationLog.correctedObjects as any[])) {
                annotations.push({
                  className: obj.class || 'plastic_bag',
                  bbox: obj.bbox || [0, 0, 100, 100],
                  confidence: 1.0,
                  annotationSource: 'OPERATOR_GROUND_TRUTH' as const
                });
              }
            } else if (snapshot && Array.isArray((snapshot as any).objects)) {
              for (const obj of ((snapshot as any).objects as any[])) {
                annotations.push({
                  className: obj.class || 'plastic_bag',
                  bbox: obj.bbox || [0, 0, 100, 100],
                  confidence: obj.confidence || 0.8,
                  annotationSource: 'AI_PREDICTION' as const
                });
              }
            }

            manifestItems.push({
              candidateId: cand._id as mongoose.Types.ObjectId,
        ...
        let status: DatasetVersionStatus = 'BUILDING';
        let structurallyValid = false;
        let trainingEligibilityStatus: TrainingEligibilityStatus = 'NOT_ELIGIBLE';
        ...
        const docs = await AiDatasetVersionModel.create(
          [
            {
              datasetVersion,
              targetModel: params.targetModel,
              ...
              manifestItems,
              createdByUserId: params.createdByUserId ? new mongoose.Types.ObjectId(params.createdByUserId) : undefined
            }
          ],
          { session: mongoSession }
        );
  ...
  public computeManifestHash(items: IDatasetManifestItem[]): string {
    const plainSanitizedItems = this.sanitizeManifestItemsForHash(items);
    const canonicalManifestJson = this.canonicalStringify(plainSanitizedItems);
    return crypto.createHash('sha256').update(canonicalManifestJson).digest('hex');
  }
}
