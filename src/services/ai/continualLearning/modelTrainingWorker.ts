...
export class ModelTrainingWorker {
  public static readonly LEASE_DURATION_MS = 60000; // 60 seconds lease

  public async claimNextJob(workerId: string): Promise<IModelTrainingJob | null> {
    const now = new Date();
    ...
    const job = await ModelTrainingJobModel.findOneAndUpdate(
      {
        $or: [
          { status: 'QUEUED' },
          { status: 'RETRY_WAIT' },
  ...
  public async sendHeartbeat(jobId: string, workerId: string, claimToken: string): Promise<boolean> {
    const now = new Date();
    const leaseExpiresAt = new Date(now.getTime() + ModelTrainingWorker.LEASE_DURATION_MS);

    const updated = await ModelTrainingJobModel.findOneAndUpdate(
      { _id: jobId, workerId, claimToken, leaseExpiresAt: { $gt: now } },
      { $set: { leaseExpiresAt } },
      { new: true }
    ).exec();
  ...
  public async handleJobFailure(job: IModelTrainingJob, errorCode: string, errorMessage: string, isRetryable: boolean): Promise<IModelTrainingJob> {
  ...
  private async revalidateJobInputs(job: IModelTrainingJob): Promise<boolean> {
    ...
    const goldenDataset = await AiGoldenDatasetVersionModel.findOne({ goldenDatasetVersion: job.goldenDatasetVersion }).exec();
    if (!goldenDataset || goldenDataset.status !== 'APPROVED' || goldenDataset.manifestHash !== job.goldenManifestHash) {
      return false;
    }

    const policy = await TrainingEligibilityPolicyModel.findOne({ policyId: job.approvedEligibilityPolicyId }).exec();
  ...
  public async runTrainerStub(job: IModelTrainingJob): Promise<IModelTrainingJob> {
    ...
    const updateJobState = async (nextStatus: 'TRAINING' | 'EVALUATING' | 'COMPLETED' | 'CANCELLED', extraFields: any = {}) => {
      const currentJob = await ModelTrainingJobModel.findOne({ _id: job._id }).exec();
      ...
      const updated = await ModelTrainingJobModel.findOneAndUpdate(
        { _id: job._id, workerId: job.workerId, claimToken: job.claimToken },
        {
          $set: {
            status: nextStatus,
            leaseExpiresAt,
...
export const modelTrainingWorker = new ModelTrainingWorker();
