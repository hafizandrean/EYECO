...
# Let's inspect the two exact target trash regions:
# Region 1: Milk Carton on front ledge: x ~ 7-16%, y ~ 75-88%
# Region 2: Cup in garden soil: x ~ 15-25%, y ~ 64-75%

# Person box from YOLO
...
candidates.sort(key=lambda item: item['bottom_y'], reverse=True)

# Pick max 2 non-overlapping distinct trash objects
selected_boxes = []
for cand in candidates:
    ...
    for sb in selected_boxes:
        sx, sy, sw, sh = sb
        # Check IoU or distance between box centers
        dist = np.sqrt((cx - sx)**2 + (cy - sy)**2)
        if dist < 120:  # If boxes are too close to each other, reject as duplicate!
...
print(f"Final 2 Selected Boxes: {len(selected_boxes)}")
for idx, b in enumerate(selected_boxes):
    ...
    print(f"Selected Box #{idx+1}: %={[pct_x, pct_y, pct_w, pct_h]}, BBox={b}")
