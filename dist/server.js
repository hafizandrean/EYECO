...
function requiresAdmin2FA(user) {
...
app.post('/api/auth/register-desa', async (req, res) => {
    ...
    try {
        ...
        const existingDesa = await db_1.DatabaseManager.findDesaByName(namaDesa);
        if (existingDesa) {
            return res.status(400).json({ error: 'Nama desa sudah terdaftar' });
        }
        // Check if Username already exists
        const existingUser = await db_1.DatabaseManager.findUserByUsername(username);
        if (existingUser) {
            return res.status(400).json({ error: 'Username sudah digunakan' });
        }
        // Create Desa
        const newDesa = await db_1.DatabaseManager.createDesa(namaDesa);
        // Create Superadmin User for the new Desa
        const newSuperadmin = await db_1.DatabaseManager.createUser(username, password, 'superadmin', newDesa._id.toString());
        if (!newSuperadmin) {
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        if (!resolvedDesaId) {
            const defaultDesa = await db_1.DatabaseManager.findDesaByName('Desa EYECO');
            if (!defaultDesa) {
        ...
        const newUser = await db_1.DatabaseManager.createUser(username, password, resolvedRole, resolvedDesaId);
        if (!newUser) {
            return res.status(400).json({ error: 'Username sudah digunakan' });
        }
        res.status(201).json({
...
app.post('/api/auth/verify-2fa', async (req, res) => {
    ...
    try {
        ...
        const isValid = await db_1.DatabaseManager.verify2FAToken(userId, normalizedCode);
        if (!isValid) {
...
app.get('/login-2fa', async (req, res) => {
    try {
        ...
        res.sendFile(path_1.default.join(__dirname, '../public/views/login-2fa.html'));
    }
    catch (err) {
        res.redirect('/login');
    }
...
app.get('/api/export', checkAuth, async (req, res) => {
    try {
        ...
        const cursor = Report_1.ReportModel.find({ desaId: user.desaId }).sort({ id: 1 }).cursor();
        cursor.on('data', (doc) => {
