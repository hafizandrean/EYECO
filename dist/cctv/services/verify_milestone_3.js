...
const DatasetFeedback_1 = require("../../database/models/DatasetFeedback");
const ModelDeploymentLog_1 = require("../../database/models/ModelDeploymentLog");
const AiTrainingRun_1 = require("../../database/models/AiTrainingRun");
...
async function runVerification() {
    ...
    await DatasetFeedback_1.DatasetFeedbackModel.deleteMany({ reviewedBy: 'mlops-tester' });
    await ModelDeploymentLog_1.ModelDeploymentLogModel.deleteMany({ deployedBy: 'mlops-tester' });
    await AiTrainingRun_1.AiTrainingRunModel.deleteMany({ datasetVersion: 'mlops-test-ds-v1.0' });
    await AiModel_1.AiModelModel.deleteMany({ id: 'incompatible-model' });
    await AiModel_1.AiModelModel.deleteMany({ id: 'compatible-model' });
    // Resolve dummy source image path
    const sourceImageRel = '/uploads/detection_1.jpg';
    const sourceImagePath = path_1.default.join(process.cwd(), 'public', sourceImageRel);
    if (!fs_1.default.existsSync(sourceImagePath)) {
        console.error(`[TEST] Source image not found at: ${sourceImagePath}`);
        process.exit(1);
    }
    // Find or create admin user for test context
    let adminUser = await User_1.UserModel.findOne({ role: 'admin' });
    if (!adminUser) {
        adminUser = await User_1.UserModel.create({
            id: 9999,
    ...
    const mockReport = await Report_1.ReportModel.create({
        id: 8888,
    ...
    const fileBuffer = fs_1.default.readFileSync(sourceImagePath);
    ...
    const datasetsDir = path_1.default.join(process.cwd(), 'public', 'datasets');
    if (!fs_1.default.existsSync(datasetsDir)) {
        fs_1.default.mkdirSync(datasetsDir, { recursive: true });
    }
    const targetFileName = `${mockReport.id}_${calculatedHash.substring(0, 16)}${path_1.default.extname(sourceImagePath)}`;
    const targetPath = path_1.default.join(datasetsDir, targetFileName);
    ...
    const feedback = await DatasetFeedback_1.DatasetFeedbackModel.create({
    ...
    const incompatibleModel = await AiModel_1.AiModelModel.create({
    ...
    try {
        ...
        const failLog = await ModelDeploymentLog_1.ModelDeploymentLogModel.findOne({
    ...
    try {
        ...
        const successLog = await ModelDeploymentLog_1.ModelDeploymentLogModel.findOne({
    ...
    // ----------------------------------------------------
    console.log('\n--- TEST 4: Training Run Registry ---');
    const trainingRun = await AiTrainingRun_1.AiTrainingRunModel.create({
    ...
    const retrievedRun = await AiTrainingRun_1.AiTrainingRunModel.findOne({ modelVersion: 'yolov8-river-v1.2-retrained' });
    ...
    await Report_1.ReportModel.deleteOne({ id: 8888 });
    // Close DB connection
