...
router.get('/my-requests', authMiddleware_1.authMiddleware, (0, RoleMiddleware_1.roleGuard)(['user', 'operator', 'admin']), async (req, res) => {
    try {
        const userId = req.userContext?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        const requests = await JoinRequest_1.JoinRequestModel.find({ userId }).sort({ createdAt: -1 }).lean().exec();
        // Enrich with workspace name
        const enriched = await Promise.all(requests.map(async (r) => {
            const ws = await Workspace_1.WorkspaceModel.findOne({ id: r.workspaceId }, 'name company code').lean().exec();
            return {
                ...r,
...
router.get('/requests/count', authMiddleware_1.authMiddleware, (0, RoleMiddleware_1.roleGuard)(['admin', 'superadmin']), async (req, res) => {
    try {
        ...
        const count = await JoinRequest_1.JoinRequestModel.countDocuments({ workspaceId: adminWorkspaceId, status: 'PENDING' });
...
router.get('/requests', authMiddleware_1.authMiddleware, (0, RoleMiddleware_1.roleGuard)(['admin', 'superadmin']), async (req, res) => {
    try {
        const adminUser = await User_1.UserModel.findOne({ id: req.userContext?.id }).lean().exec();
        const adminWorkspaceId = adminUser?.workspaceId;
        ...
        const filterQuery = { workspaceId: requestedWsId };
        if (statusFilter && statusFilter !== 'SEMUA') {
            filterQuery.status = statusFilter;
        }
        const requests = await JoinRequest_1.JoinRequestModel.find(filterQuery).sort({ createdAt: -1 }).lean().exec();
        // Enrich with user and decision details
        const enrichedRequests = await Promise.all(requests.map(async (reqItem) => {
            const user = await User_1.UserModel.findOne({ id: reqItem.userId }, 'name username email phone role').lean().exec();
            let deciderName = '-';
            if (reqItem.decidedBy) {
                const decider = await User_1.UserModel.findOne({ id: reqItem.decidedBy }, 'name username').lean().exec();
...
router.post('/requests/:id/decide', authMiddleware_1.authMiddleware, (0, RoleMiddleware_1.roleGuard)(['admin', 'superadmin']), async (req, res) => {
    try {
        ...
        let joinReq = await JoinRequest_1.JoinRequestModel.findById(reqId);
        if (!joinReq) {
            joinReq = await JoinRequest_1.JoinRequestModel.findOne({ id: Number(reqId) });
        }
        if (!joinReq) {
        ...
        if (action === 'APPROVED') {
            const targetUser = await User_1.UserModel.findOne({ id: joinReq.userId });
            if (targetUser) {
                const wsIds = targetUser.workspaceIds || [];
        ...
        try {
            const targetUser = await User_1.UserModel.findOne({ id: joinReq.userId }).lean().exec();
            const ws = await Workspace_1.WorkspaceModel.findOne({ id: joinReq.workspaceId }).lean().exec();
            if (targetUser && ws) {
...
router.post('/requests/:id/approve', authMiddleware_1.authMiddleware, (0, RoleMiddleware_1.roleGuard)(['admin', 'superadmin']), (req, res, next) => {
    req.body.action = 'APPROVED';
    next();
}, router);
router.post('/requests/:id/reject', authMiddleware_1.authMiddleware, (0, RoleMiddleware_1.roleGuard)(['admin', 'superadmin']), (req, res, next) => {
...
router.post('/select', authMiddleware_1.authMiddleware, (0, RoleMiddleware_1.roleGuard)(['user', 'operator', 'admin']), async (req, res) => {
...
router.post('/join', authMiddleware_1.authMiddleware, (0, RoleMiddleware_1.roleGuard)(['user', 'operator']), async (req, res) => {
    const { workspaceId } = req.body;
    if (!workspaceId)
