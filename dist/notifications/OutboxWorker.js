...
class OutboxWorker {
    ...
    static async processQueue() {
        ...
        try {
            ...
            const unsentValidReports = await Report_1.ReportModel.find({
                adminStatus: 'VALID',
            ...
            for (const report of unsentValidReports) {
                ...
                const existingOutbox = await OutboxEvent_1.OutboxEventModel.findOne({ idempotencyKey }).exec();
     ...
     */
    static async reconcileProjections() {
        let reconciledCount = 0;
        try {
            // Find all PROCESSED events where Report.telegramStatus is not SENT
            const processedEvents = await OutboxEvent_1.OutboxEventModel.find({
                aggregateType: 'Report',
                eventType: 'REPORT_VALIDATED_TELEGRAM',
                status: 'PROCESSED'
            }).exec();
            for (const event of processedEvents) {
                ...
                const report = await Report_1.ReportModel.findOne({ id: reportId }).exec();
            ...
            const failedEvents = await OutboxEvent_1.OutboxEventModel.find({
                aggregateType: 'Report',
                eventType: 'REPORT_VALIDATED_TELEGRAM',
                status: 'FAILED'
            }).exec();
