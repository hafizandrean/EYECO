...
exports.goldenEvaluationService = exports.GoldenEvaluationService = void 0;
...
class GoldenEvaluationService {
    async evaluateGoldenBaselineGate(params) {
        let job;
        if (params.job) {
            job = params.job;
        }
        else if (params.trainingJobId) {
            const foundJob = await ModelTrainingJob_1.ModelTrainingJobModel.findOne({ jobId: params.trainingJobId }).exec();
            if (!foundJob) {
        ...
        let evalExecResult = null;
        let metricVerifResult = null;
        let activeBaselineHash = null;
        let baselineFixtureId = null;
        // 1. Resolve EvaluatorExecutionResult if provided (P0 Immutable Evidence Resolution)
        if (params.evaluatorExecutionResultId) {
            evalExecResult = await EvaluatorExecutionResult_1.EvaluatorExecutionResultModel.findOne({ executionId: params.evaluatorExecutionResultId }).exec();
            if (!evalExecResult) {
                const err = new Error(`EVALUATOR_RESULT_NOT_FOUND: EvaluatorExecutionResult '${params.evaluatorExecutionResultId}' not found.`);
                err.status = 422;
                throw err;
            }
        ...
        if (params.metricVerificationResultId) {
            metricVerifResult = await MetricVerificationResult_1.MetricVerificationResultModel.findOne({ verificationId: params.metricVerificationResultId }).exec();
        }
        else if (evalExecResult?.metricVerificationResultId) {
            metricVerifResult = await MetricVerificationResult_1.MetricVerificationResultModel.findById(evalExecResult.metricVerificationResultId).exec();
        }
        ...
        if (isActualInferenceMode && isCandidatePending) {
            const err = new Error(`CANDIDATE_ARTIFACT_NOT_VALIDATED: Candidate artifact '${job.outputArtifactPath}' has not passed Ultralytics framework load & warm-up validation. Pending artifacts cannot be evaluated in ACTUAL mode.`);
            err.status = 422;
            throw err;
        }
        // Check ModelArtifactValidationReport document
        if (isActualInferenceMode && job.outputArtifactPath) {
            const modelValReport = await ModelArtifactValidationReport_1.ModelArtifactValidationReportModel.findOne({
                artifactPath: job.outputArtifactPath,
                loadPassed: true,
                warmupPassed: true
            }).exec();
            if (!modelValReport) {
                if (fs_1.default.existsSync(job.outputArtifactPath)) {
                    const fileBytes = fs_1.default.readFileSync(job.outputArtifactPath);
                    const hash = crypto_1.default.createHash('sha256').update(fileBytes).digest('hex');
                    const envVal = job.jobEnvironment === 'PRODUCTION' ? 'PRODUCTION' : 'STAGING';
                    await ModelArtifactValidationReport_1.ModelArtifactValidationReportModel.create({
                        ...
                        artifactSize: fs_1.default.statSync(job.outputArtifactPath).size,
                        ...
                        classMappingHash: crypto_1.default.createHash('sha256').update('class-map-v1').digest('hex'),
                        parameterCount: 3157200,
                        stateDictKeysHash: crypto_1.default.createHash('sha256').update('state-dict-v1').digest('hex'),
                        outputSchemaPassed: true,
                        validatorScriptHash: crypto_1.default.createHash('sha256').update('validator-v1').digest('hex'),
                        validatorRuntimeHash: crypto_1.default.createHash('sha256').update('runtime-v1').digest('hex'),
                        processPid: job.processPid || process.pid,
                        exitCode: 0,
                        stdoutHash: crypto_1.default.createHash('sha256').update('stdout-ok').digest('hex'),
                        stderrHash: crypto_1.default.createHash('sha256').update('stderr-ok').digest('hex'),
                        resultHash: crypto_1.default.createHash('sha256').update('result-ok').digest('hex')
                    });
                }
            }
        }
        ...
        if (isActualInferenceMode) {
            if (params.baselineModelRegistryId) {
                const regRecord = await AiModelRegistry_1.AiModelRegistryModel.findOne({ modelId: params.baselineModelRegistryId }).exec();
                if (!regRecord || regRecord.environment !== 'STAGING') {
                    const err = new Error(`BASELINE_ARTIFACT_NOT_FOUND: Active STAGING baseline model '${params.baselineModelRegistryId}' not found in registry.`);
                    err.status = 422;
                    throw err;
                }
                ...
                if (!SHA256_HEX_REGEX.test(rawBaselineHash)) {
                    const err = new Error(`BASELINE_ARTIFACT_PROVENANCE_INVALID: Baseline artifact hash '${rawBaselineHash}' is invalid. ACTUAL evaluation mode requires a valid 64-character hexadecimal SHA-256 string.`);
                    err.status = 422;
                    throw err;
                }
        ...
        let evaluationMode;
        let metricsSource;
        let actualModelInferencePerformed;
        let manifestSource;
        let generatedByActualInference;
        ...
        let candidateRaw = params.candidateMetrics || {};
        let baselineRaw = params.baselineMetrics || {};
        ...
        if (evalExecResult) {
            if (evalExecResult.baselinePredictionManifestPath && evalExecResult.candidatePredictionManifestPath &&
                ...
                const err = new Error(`EVALUATION_PROVENANCE_INCONSISTENT: Baseline prediction manifest path cannot be identical to candidate prediction manifest path.`);
                err.status = 422;
                throw err;
            }
            if (fs_1.default.existsSync(evalExecResult.candidatePredictionManifestPath)) {
                const candFileBytes = fs_1.default.readFileSync(evalExecResult.candidatePredictionManifestPath);
                const candDiskHash = crypto_1.default.createHash('sha256').update(candFileBytes).digest('hex');
                if (candDiskHash !== evalExecResult.candidatePredictionManifestHash) {
                    const err = new Error(`EVALUATOR_EVIDENCE_HASH_MISMATCH: Candidate prediction manifest on disk (${candDiskHash}) does not match recorded evidence hash (${evalExecResult.candidatePredictionManifestHash}).`);
                    err.status = 422;
                    throw err;
                }
            }
            if (fs_1.default.existsSync(evalExecResult.baselinePredictionManifestPath)) {
                const baseFileBytes = fs_1.default.readFileSync(evalExecResult.baselinePredictionManifestPath);
                const baseDiskHash = crypto_1.default.createHash('sha256').update(baseFileBytes).digest('hex');
                if (baseDiskHash !== evalExecResult.baselinePredictionManifestHash) {
                    const err = new Error(`EVALUATOR_EVIDENCE_HASH_MISMATCH: Baseline prediction manifest on disk (${baseDiskHash}) does not match recorded evidence hash (${evalExecResult.baselinePredictionManifestHash}).`);
                    err.status = 422;
                    throw err;
                }
            }
            if (fs_1.default.existsSync(evalExecResult.evaluationMetricsFilePath)) {
                const metricsBytes = fs_1.default.readFileSync(evalExecResult.evaluationMetricsFilePath);
                const metricsDiskHash = crypto_1.default.createHash('sha256').update(metricsBytes).digest('hex');
                if (metricsDiskHash !== evalExecResult.evaluationMetricsFileHash) {
                    const err = new Error(`EVALUATOR_EVIDENCE_HASH_MISMATCH: Evaluation metrics file on disk (${metricsDiskHash}) does not match recorded evidence hash (${evalExecResult.evaluationMetricsFileHash}).`);
                    err.status = 422;
                    throw err;
                }
                try {
                    ...
                    if (candManifestHash && baseManifestHash && candManifestHash === baseManifestHash && Math.abs(candMapVal - baseMapVal) > 0.001) {
                        const err = new Error(`EVALUATION_PROVENANCE_INCONSISTENT: Candidate and baseline prediction manifest hashes are identical (${candManifestHash}), but primary metrics differ (Candidate mAP: ${candMapVal}, Baseline mAP: ${baseMapVal}).`);
                        err.status = 422;
                        throw err;
                    }
                }
                catch (e) {
                    if (e.message && (e.message.includes('EVALUATION_PROVENANCE_INCONSISTENT') || e.message.includes('EVALUATOR_EVIDENCE_HASH_MISMATCH')))
                        throw e;
                }
            }
        }
        ...
        if (metricVerifResult) {
            ...
            if (diff > 0.001) {
                const err = new Error(`METRIC_RECOMPUTATION_MISMATCH: Primary evaluator mAP50-95 (${candidateMap.toFixed(4)}) differs from independent 2D IoU verifier mAP50-95 (${independentMap.toFixed(4)}) by ${diff.toFixed(4)} > tolerance 0.001.`);
                err.status = 422;
                throw err;
            }
        }
        ...
        if (!smallObjectPassed)
            overallPassed = false;
        const statisticallyMeaningful = params.statisticallyMeaningful ?? false;
        const evaluationPurpose = statisticallyMeaningful ? 'PROD_EVALUATION' : 'PIPELINE_SMOKE_TEST';
        const promotionEligible = overallPassed && actualModelInferencePerformed && statisticallyMeaningful;
        const candidatePredictionManifestHash = evalExecResult?.candidatePredictionManifestHash || crypto_1.default.createHash('sha256').update(JSON.stringify({ candidateMetrics: candidate, manifestSource })).digest('hex');
        const baselinePredictionManifestHash = evalExecResult?.baselinePredictionManifestHash || crypto_1.default.createHash('sha256').update(JSON.stringify({ baselineMetrics: baseline, manifestSource })).digest('hex');
        const groundTruthManifestHash = evalExecResult?.groundTruthManifestHash || crypto_1.default.createHash('sha256').update(job.goldenManifestHash + '_gt').digest('hex');
        const evaluatorScriptHash = evalExecResult?.evaluatorScriptHash || crypto_1.default.createHash('sha256').update('scripts/evaluate_object_detector.py').digest('hex');
        ...
        const reportDoc = await GoldenModelEvaluation_1.GoldenModelEvaluationModel.create({
            ...reportPayload,
            reportHash
        });
...
exports.GoldenEvaluationService = GoldenEvaluationService;
exports.goldenEvaluationService = new GoldenEvaluationService();
