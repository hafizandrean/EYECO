...
class RuleBasedStrategy {
    evaluate(fv) {
        ...
        let score = 0;
        if (fv.personCount > 0)
            score += (w.personDetected || 10);
        else
            score += (w.noPersonDetected || -15);
        if (fv.trashCount > 0) {
            score += (w.trashDetected || 25);
            if (fv.highestTrashConfidence >= 75)
                score += (w.trashHighConfidence || 10);
        }
        else {
        ...
        const calculatedConfidence = Math.round(evidenceCoverageFactor + objectReliabilityFactor + sceneReliabilityFactor + analyzerAgreementFactor + imageQualityFactor);
